...
class RedisChatMessageHistory:
    ...
    def _load_local(self) -> None:
        ...
        try:
            ...
            self._store = defaultdict(list, {k: list(v) for k, v in payload.items()})
        except Exception:
            self._store = defaultdict(list)
    ...
    def add_user_message(self, user_id: str, message: str) -> None:
        item = json.dumps({"role": "user", "content": message}, ensure_ascii=False)
        if self._use_redis and self._client:
            self._client.rpush(f"chat:{user_id}", item)
        else:
            self._store[user_id].append({"role": "user", "content": message})
            self._save_local()

    def add_ai_message(self, user_id: str, message: str) -> None:
        item = json.dumps({"role": "assistant", "content": message}, ensure_ascii=False)
        if self._use_redis and self._client:
            self._client.rpush(f"chat:{user_id}", item)
        else:
            self._store[user_id].append({"role": "assistant", "content": message})
            self._save_local()

    def get_messages(self, user_id: str) -> List[dict[str, str]]:
        if self._use_redis and self._client:
            items = self._client.lrange(f"chat:{user_id}", 0, -1)
            return [json.loads(x) for x in items]
        return list(self._store.get(user_id, []))

    def add_system_message(self, user_id: str, message: str) -> None:
        item = json.dumps({"role": "system", "content": message}, ensure_ascii=False)
        if self._use_redis and self._client:
            self._client.rpush(f"chat:{user_id}", item)
        else:
            self._store[user_id].append({"role": "system", "content": message})
            self._save_local()
